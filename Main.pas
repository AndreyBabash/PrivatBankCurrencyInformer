...
begin
  ...
  begin
    MainListView.Items.Add.Text:='Курс валют по отношению к UAH'+#13#10+#13#10+#13#10+#13#10;

    if (TJSONPair(TJSONObject(JSONMyArr.Get(i)).Get('base_ccy')).JsonValue.Value)='USD' then
      begin
         MainListView.Items[i].Detail:='Код валюты: '+(TJSONPair(TJSONObject(JSONMyArr.Get(i)).Get('ccy')).JsonValue.Value)+#13#10+
          'Код национальной валюты: '+(TJSONPair(TJSONObject(JSONMyArr.Get(i)).Get('base_ccy')).JsonValue.Value)+#13#10+
          Format('Курс покупки:  %n', [(TJSONPair(TJSONObject(JSONMyArr.Get(i)).Get('buy')).JsonValue.Value.ToSingle)])+' USD'+#13#10+
          Format('Курс продажи:  %n', [(TJSONPair(TJSONObject(JSONMyArr.Get(i)).Get('sale')).JsonValue.Value.ToSingle)])+' USD';
      end
      else
      begin
        MainListView.Items[i].Detail:='Код валюты: '+(TJSONPair(TJSONObject(JSONMyArr.Get(i)).Get('ccy')).JsonValue.Value)+#13#10+
          'Код национальной валюты: '+(TJSONPair(TJSONObject(JSONMyArr.Get(i)).Get('base_ccy')).JsonValue.Value)+#13#10+
          Format('Курс покупки:  %n', [(TJSONPair(TJSONObject(JSONMyArr.Get(i)).Get('buy')).JsonValue.Value.ToSingle)])+' UAH'+#13#10+
          Format('Курс продажи:  %n', [(TJSONPair(TJSONObject(JSONMyArr.Get(i)).Get('sale')).JsonValue.Value.ToSingle)])+' UAH';
      end;
